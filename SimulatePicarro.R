...
SD.MACHINE     <- 0.025 #standard deviation of random measurement error assumed to be iid and gaussian [Value from PICARRO]
DRIFT.MACHINE  <- 0.2 / 24 / 30 #linear Drift in permille per hour, estimate by hand from the experiment [Value from PICARRO would be 0.2 permille per 24h = 0.2/24]
MEMORY.MACHINE <- 0.5 #Memory coefficient of the machine

DRIFT.VIAL     <- 0.02 / 100 #Linear Drift in permille per hour and per hole in the Septum
...
DHOLE.VIAL     <- 0.0 #Not every injection will create a new hole; this coefficient determines the increase of the Septum holes per injection
...
##' @param vial dataframe with columns d18O (in permille),water.volume (in ml),holes (#holes in the septum)
##' @param sequence vector giving the sequence of the measurements
##' @param names vector with the same length as sequence, providing names for the blocks; not used in the simulation but gets output in Analysis
...
simulate <- function(vial, sequence, names, d18O.machine = -37.5)
{
    ##Initial values
    runtime.minutes <- 0
    ...
    for (i.step in 1:length(sequence))
    {
        ...
        vial$d18O<-vial$d18O + vial$holes*DRIFT.VIAL*TIME.INJECTION/60
        ...
        measured <- vial$d18O[i.vial] + rnorm(1,sd=SD.MACHINE) + DRIFT.MACHINE*(runtime.minutes/60) - (vial$d18O[i.vial]-d18O.machine) * MEMORY.MACHINE


        d18O.machine <- measured

        ##Save the output
        output$Line[i.step]<-i.step
        output$Analysis[i.step]<-names[i.step]
        output$Port[i.step]<-paste(i.vial)
        output$d.18_16.Mean[i.step]<-measured
    }
...
vial<-data.frame(d18O=c(-38.8,-38.8,-38.8,-38.8,-38.8,-38.8),water.volume=c(1.6,1.6,1.6,1.6,1.6,1.6),holes=c(0,0,0,0,0,0))
